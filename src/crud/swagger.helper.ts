...
import { CrudOptions, ParamsOptions } from '../interfaces';
...
export class Swagger {
  static operationsMap(modelName: string): { [key in BaseRouteName]: string } {
    return {
      getMany: `Retrieve multiple ${pluralize(modelName)}`,
      getOne: `Retrieve a single ${modelName}`,
      createMany: `Create multiple ${pluralize(modelName)}`,
      createOne: `Create a single ${modelName}`,
      updateOne: `Update a single ${modelName}`,
      deleteOne: `Delete a single ${modelName}`,
      recoverOne: `Recover one ${modelName}`,
    };
  }
  ...
  static createResponseMeta(
    ...
    if (swagger) {
      ...
      switch (name) {
        ...
        default:
          const dto = swaggerModels[name.split('One')[0]];
